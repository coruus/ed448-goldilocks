...
int test_goldilocks() {
  ...
  unsigned char ss12[GOLDI_SHARED_SECRET_BYTES], ss21[GOLDI_SHARED_SECRET_BYTES],
      ss21p[GOLDI_SHARED_SECRET_BYTES], proto[GOLDI_SYMKEY_BYTES];
  ...
  for (i = 0; i < 1000 && good; i++) {
    ...
    ret = goldilocks_sign(
        signature, (const unsigned char*)message1, strlen(message1), &priv);
    if (ret) {
      youfail();
    ...
    ret = goldilocks_verify_precomputed(
        signature, (const unsigned char*)message1, strlen(message1), pre);
    if (ret) {
      youfail();
    ...
    ret = goldilocks_verify(
        signature, (const unsigned char*)message2, strlen(message1), &pub);
    if (ret != GOLDI_EINVAL) {
      youfail();
    ...
    ret = goldilocks_verify_precomputed(
        signature, (const unsigned char*)message2, strlen(message1), pre);
    if (ret != GOLDI_EINVAL) {
      youfail();
    ...
    ret = goldilocks_shared_secret_precomputed(
        ss21p, GOLDI_SHARED_SECRET_BYTES, &priv2, pre);
    if (ret) {
      youfail();
