...
static word_t widemac(word_t* accum,
...
void barrett_reduce(word_t* a,
  ...
  for (; nwords_left_in_a >= prime->nwords_p; nwords_left_in_a--) {
    for (repeat = 0; repeat < 2; repeat++) {
      ...
      word_t carry = widemac(a + nwords_left_in_a - prime->nwords_p,
                             prime->nwords_p,
                             prime->p_lo,
                             prime->nwords_lo,
                             mand,
...
void barrett_mul_or_mac(word_t* accum,
  ...
  for (bpos = nwords_b - 1; bpos >= 0; bpos--) {
    ...
    /* the mac can't carry, because nwords_tmp >= nwords_a+1 and its high word
     * is clear */
    assert(!carry);
