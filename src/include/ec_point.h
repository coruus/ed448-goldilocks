 ...
 */
static __inline__ void copy_affine(struct affine_t* a, const struct affine_t* ds)
    __attribute__((unused, always_inline));

/**
 * Auto-generated copy method.
 */
static __inline__ void copy_tw_affine(struct tw_affine_t* a, const struct tw_affine_t* ds)
    __attribute__((unused, always_inline));

/**
 * Auto-generated copy method.
 */
static __inline__ void copy_montgomery(struct montgomery_t* a,
 ...
 * Auto-generated copy method.
 */
static __inline__ void copy_extensible(struct extensible_t* a,
 ...
 * Auto-generated copy method.
 */
static __inline__ void copy_tw_extensible(struct tw_extensible_t* a,
 ...
 * Auto-generated copy method.
 */
static __inline__ void copy_tw_niels(struct tw_niels_t* a, const struct tw_niels_t* ds)
    __attribute__((unused, always_inline));

/**
 * Auto-generated copy method.
 */
static __inline__ void copy_tw_pniels(struct tw_pniels_t* a, const struct tw_pniels_t* ds)
    __attribute__((unused, always_inline));
 ...
 * If x=0, returns 0.
 */
void p448_isr(struct p448_t* a, const struct p448_t* x);
 ...
 * If x=0, returns 0.
 */
void p448_inverse(struct p448_t* a, const struct p448_t* x);

/**
 * Add two points on a twisted Edwards curve, one in Extensible form
 * and the other in half-Niels form.
 */
void add_tw_niels_to_tw_extensible(struct tw_extensible_t* d, const struct tw_niels_t* e);

/**
 * Add two points on a twisted Edwards curve, one in Extensible form
 * and the other in half-Niels form.
 */
void sub_tw_niels_from_tw_extensible(struct tw_extensible_t* d,
 ...
 */
void add_tw_pniels_to_tw_extensible(struct tw_extensible_t* e,
 ...
 * and the other in projective Niels form.
 */
void sub_tw_pniels_from_tw_extensible(struct tw_extensible_t* e,
                                      const struct tw_pniels_t* a);

/**
 * Double a point on a twisted Edwards curve, in "extensible" coordinates.
 */
void double_tw_extensible(struct tw_extensible_t* a);

/**
 * Double a point on an Edwards curve, in "extensible" coordinates.
 */
void double_extensible(struct extensible_t* a);
 ...
 */
void twist_and_double(struct tw_extensible_t* b, const struct extensible_t* a);
 ...
 */
void serialize_extensible(struct p448_t* b, const struct extensible_t* a);
...
/**
 * Expensive transfer from untwisted to twisted.  Roughly equivalent to halve
 *and isogeny.
 * Correctly transfers point of order 2.
 *
 * Can't have x=+1 (it's not even).  There is code to fix the exception that
 *would otherwise
 * occur at (0,1).
 *
 ...
 *and isogeny.
 *
 * This function is for testing purposes only, because it can return odd points
 *on the
 * twist.  This can cause exceptions in the point addition formula.  What's
 *more, this
 * function should be able to return points of order 4, which are at infinity.
 *
 * This function probably doesn't properly handle special cases, such as the
 *point at
 * infinity (FUTURE).
 *
 * This function probably isn't a homomorphism, in that it probably doesn't
 *consistently
...
mask_t is_square(const struct p448_t* x);
 ...
 */
mask_t deserialize_affine(struct affine_t* a, const struct p448_t* sz);
 ...
 */
mask_t deserialize_and_twist_approx(struct tw_extensible_t* a,
                                    const struct p448_t* sdm1,
                                    const struct p448_t* sz);

void set_identity_extensible(struct extensible_t* a);

void set_identity_tw_extensible(struct tw_extensible_t* a);

void set_identity_affine(struct affine_t* a);

mask_t eq_affine(const struct affine_t* a, const struct affine_t* b);

mask_t eq_extensible(const struct extensible_t* a, const struct extensible_t* b);

mask_t eq_tw_extensible(const struct tw_extensible_t* a, const struct tw_extensible_t* b);

void elligator_2s_inject(struct affine_t* a, const struct p448_t* r);

mask_t validate_affine(const struct affine_t* a);
 ...
 */
mask_t validate_tw_extensible(const struct tw_extensible_t* ext);
 ...
 * with no regard for speed.
 */
mask_t validate_extensible(const struct extensible_t* ext);

void copy_affine(struct affine_t* a, const struct affine_t* ds) {
  p448_copy(&a->x, &ds->x);
  p448_copy(&a->y, &ds->y);
}

void copy_tw_affine(struct tw_affine_t* a, const struct tw_affine_t* ds) {
  p448_copy(&a->x, &ds->x);
  p448_copy(&a->y, &ds->y);
}

void copy_montgomery(struct montgomery_t* a, const struct montgomery_t* ds) {
...
void copy_extensible(struct extensible_t* a, const struct extensible_t* ds) {
  p448_copy(&a->x, &ds->x);
  p448_copy(&a->y, &ds->y);
  p448_copy(&a->z, &ds->z);
  p448_copy(&a->t, &ds->t);
...
void copy_tw_extensible(struct tw_extensible_t* a, const struct tw_extensible_t* ds) {
  p448_copy(&a->x, &ds->x);
  p448_copy(&a->y, &ds->y);
  p448_copy(&a->z, &ds->z);
  p448_copy(&a->t, &ds->t);
...
void copy_tw_niels(struct tw_niels_t* a, const struct tw_niels_t* ds) {
  p448_copy(&a->a, &ds->a);
  p448_copy(&a->b, &ds->b);
  p448_copy(&a->c, &ds->c);
}

void copy_tw_pniels(struct tw_pniels_t* a, const struct tw_pniels_t* ds) {
