...
static __inline__ void quarter_round_ssse3(ssereg* a, ssereg* b, ssereg* c, ssereg* d) {
  ...
  *c = add(*c, *d);
  *b = sse2_rotate(12, *b ^ *c);
  *a = add(*a, *b);
  *d = ssse3_rotate_8(*d ^ *a);
  *c = add(*c, *d);
  *b = sse2_rotate(7, *b ^ *c);
}
...
static __inline__ __attribute__((unused)) void quarter_round(u_int32_t* a,
                                                             u_int32_t* b,
                                                             u_int32_t* c,
                                                             u_int32_t* d) {
  *a = *a + *b;
...
static void crandom_chacha_expand(u_int64_t iv,
                                  u_int64_t ctr,
                                  int nr,
                                  int output_size,
...
int crandom_generate(struct crandom_state_t* state,
  ...
  /* the generator isn't seeded; maybe they ignored the return value of
   * init_from_file */
      ...
      if (state->reseed_interval) {
        ...
        if (unlikely(state->reseed_countdown <= 0)) {
          ...
          if (offset < 32) {
             ...
             *
             * If not, the generator is still probably safe to use, because
             *reseeding
             * is basically over-engineering for caution.  Also, the user might
             *ignore
             * the return code, so we still need to fill the request.
             *
             * Set reseed_countdown = 1 so we'll try again later.  If the user's
             * performance sucks as a result of ignoring the error code while
             *calling
